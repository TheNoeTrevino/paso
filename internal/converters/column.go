package converters

import (
	"github.com/thenoetrevino/paso/internal/database/types"
	"github.com/thenoetrevino/paso/internal/models"
)

// ColumnToModel converts a types.Column (SQLC database model) to models.Column (domain model).
//
// Handles nullable foreign keys for linked list structure:
// - prev_id (types.NullInt64 → *int) - Previous column in workflow
// - next_id (types.NullInt64 → *int) - Next column in workflow
//
// Special flags indicate column purpose:
// - HoldsReadyTasks: Column contains tasks ready to work on
// - HoldsCompletedTasks: Column contains finished tasks
// - HoldsInProgressTasks: Column contains tasks currently being worked on
//
// Type conversions:
// - All ID fields: int64 → int
// - Nullable IDs: types.NullInt64 → *int
func ColumnToModel(c types.Column) *models.Column {
	var prevID, nextID *int
	if c.PrevID.Valid {
		val := int(c.PrevID.Int64)
		prevID = &val
	}
	if c.NextID.Valid {
		val := int(c.NextID.Int64)
		nextID = &val
	}

	return &models.Column{
		ID:                   int(c.ID),
		Name:                 c.Name,
		ProjectID:            int(c.ProjectID),
		PrevID:               prevID,
		NextID:               nextID,
		HoldsReadyTasks:      c.HoldsReadyTasks,
		HoldsCompletedTasks:  c.HoldsCompletedTasks,
		HoldsInProgressTasks: c.HoldsInProgressTasks,
	}
}

// ColumnFromIDRowToModel converts a types.GetColumnByIDRow to models.Column.
// This handles the specific row type returned by the GetColumnByID query.
// See ColumnToModel for field conversion details.
func ColumnFromIDRowToModel(r types.GetColumnByIDRow) *models.Column {
	var prevID, nextID *int
	if r.PrevID.Valid {
		val := int(r.PrevID.Int64)
		prevID = &val
	}
	if r.NextID.Valid {
		val := int(r.NextID.Int64)
		nextID = &val
	}

	return &models.Column{
		ID:                   int(r.ID),
		Name:                 r.Name,
		ProjectID:            int(r.ProjectID),
		PrevID:               prevID,
		NextID:               nextID,
		HoldsReadyTasks:      r.HoldsReadyTasks,
		HoldsCompletedTasks:  r.HoldsCompletedTasks,
		HoldsInProgressTasks: r.HoldsInProgressTasks,
	}
}

// ColumnsFromRowsToModels converts a slice of types.GetColumnsByProjectRow to a slice of models.Column.
// This handles the specific row type returned by the GetColumnsByProject query.
// See ColumnToModel for field conversion details.
func ColumnsFromRowsToModels(rows []types.GetColumnsByProjectRow) []*models.Column {
	result := make([]*models.Column, len(rows))
	for i, r := range rows {
		var prevID, nextID *int
		if r.PrevID.Valid {
			val := int(r.PrevID.Int64)
			prevID = &val
		}
		if r.NextID.Valid {
			val := int(r.NextID.Int64)
			nextID = &val
		}

		result[i] = &models.Column{
			ID:                   int(r.ID),
			Name:                 r.Name,
			ProjectID:            int(r.ProjectID),
			PrevID:               prevID,
			NextID:               nextID,
			HoldsReadyTasks:      r.HoldsReadyTasks,
			HoldsCompletedTasks:  r.HoldsCompletedTasks,
			HoldsInProgressTasks: r.HoldsInProgressTasks,
		}
	}
	return result
}
