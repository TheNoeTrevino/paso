...
func TestGetTaskTreeByProject_SortedByTicketNumber(t *testing.T) {
	...
	_, err := env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 3 WHERE id = ?", task1)
	require.NoError(t, err)
	_, err = env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 1 WHERE id = ?", task2)
	require.NoError(t, err)
	_, err = env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 2 WHERE id = ?", task3)
	require.NoError(t, err)
...
func TestGetTaskTreeByProject_MultipleRoots(t *testing.T) {
	...
	_, err := env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 1 WHERE id = ?", task1)
	require.NoError(t, err)
	_, err = env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 2 WHERE id = ?", task2)
	require.NoError(t, err)
	_, err = env.DB.ExecContext(env.Ctx, "UPDATE tasks SET ticket_number = 3 WHERE id = ?", task3)
	require.NoError(t, err)
