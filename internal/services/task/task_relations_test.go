...
func TestAddParentRelation(t *testing.T) {
	...
	fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
		...
		var count int
		err = env.DB.QueryRowContext(env.Ctx, fmt.Sprintf("SELECT COUNT(*) FROM task_subtasks WHERE parent_id = %s AND child_id = %s", d.Placeholder(1), d.Placeholder(2)), task1.ID, task2.ID).Scan(&count)
