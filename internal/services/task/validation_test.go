...
func TestValidateEstimate(t *testing.T) {
	...
	tests := []struct {
		...
		// Invalid cases - wrong units
		{
			...
			estimate: strPtr("1x"),
			wantErr:  true,
			errType:  ErrInvalidEstimateUnit,
			errMsg:   "'x' is not valid",
		},
		{
			...
			errMsg:   "expected digit at position 1",
		},
		{
			name:     "invalid unit y",
			estimate: strPtr("5y"),
		...
		},
		{
			...
			wantErr:  true,
			errType:  ErrInvalidEstimateFormat,
			errMsg:   "missing unit after number",
		},
		{
		...
		},
		{
			...
			wantErr:  true,
			errType:  ErrInvalidEstimateFormat,
			errMsg:   "expected digit at position 3",
		},
		{
