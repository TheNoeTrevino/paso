...
func TestCreateTask_WithLabels(t *testing.T) {
	...
	fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
		...
		var count int
		err = env.DB.QueryRowContext(env.Ctx, fmt.Sprintf("SELECT COUNT(*) FROM task_labels WHERE task_id = %s", d.Placeholder(1)), result.ID).Scan(&count)
...
func TestUpdateTask_Validation(t *testing.T) {
	...
	tests := []struct {
		...
		wantErr bool
		errType error
		setupFn func(*sql.DB, fixtures.Dialect, database.DatabaseType) int // Returns task ID if needed
	}{
		{
			name:    "empty title",
			title:   new(""),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
				...
				taskID := tt.taskID
				if tt.setupFn != nil {
					taskID = tt.setupFn(env.DB, d, dbType)
				}
				req := UpdateTaskRequest{
					TaskID: taskID,
					Title:  tt.title,
...
func TestAttachLabel(t *testing.T) {
	...
	fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
		...
		var count int
		err = env.DB.QueryRowContext(env.Ctx, fmt.Sprintf("SELECT COUNT(*) FROM task_labels WHERE task_id = %s AND label_id = %s", d.Placeholder(1), d.Placeholder(2)), created.ID, labelID).Scan(&count)
		require.NoError(t, err, "failed to query task labels")
