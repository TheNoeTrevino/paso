...
func TestUpdateTaskDueDate(t *testing.T) {
	...
	fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
		...
		t.Run("set due date on task without due date", func(t *testing.T) {
			taskID := fixtures.CreateTestTask(t, db, d, columnID, "Task without due date")
			...
			err = db.QueryRowContext(ctx,
				fmt.Sprintf("SELECT due_date FROM tasks WHERE id = %s", d.Placeholder(1)), taskID).Scan(&dbDueDate)
			require.NoError(t, err)
		...
		t.Run("update existing due date", func(t *testing.T) {
			taskID := fixtures.CreateTestTask(t, db, d, columnID, "Task with due date")
		...
		t.Run("set due date in the past", func(t *testing.T) {
			taskID := fixtures.CreateTestTask(t, db, d, columnID, "Task with past due date")
		...
		t.Run("set due date far in the future", func(t *testing.T) {
			taskID := fixtures.CreateTestTask(t, db, d, columnID, "Task with far future date")
		...
		t.Run("set and clear due date multiple times", func(t *testing.T) {
			taskID := fixtures.CreateTestTask(t, db, d, columnID, "Task for toggling")
