...
func TestMoveTaskUp(t *testing.T) {
	...
	fixtures.RunDatabaseTests(t, func(t *testing.T, db *sql.DB, d fixtures.Dialect, dbType database.DatabaseType) {
		...
		var pos1, pos2 int64
		err = env.DB.QueryRowContext(env.Ctx, fmt.Sprintf("SELECT position FROM tasks WHERE id = %s", d.Placeholder(1)), task1.ID).Scan(&pos1)
		require.NoError(t, err)

		err = env.DB.QueryRowContext(env.Ctx, fmt.Sprintf("SELECT position FROM tasks WHERE id = %s", d.Placeholder(1)), task2.ID).Scan(&pos2)
		require.NoError(t, err)
