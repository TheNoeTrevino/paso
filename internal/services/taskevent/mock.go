...
// This mock is intended for use within service-level tests.
// For cross-package tests (CLI, TUI, etc.), use mocks.MockTaskEventService instead.
...
// HasCallWithId checks if a method was called with the given task ID.
func (m *MockService) HasCallWithId(method string, taskID int) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
