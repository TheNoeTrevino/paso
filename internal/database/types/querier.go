// Package types provides database-agnostic interfaces and
// This file defines the Querier interface that abstracts over different database implementations
// (SQLite, PostgreSQL, etc.) by using custom types from this package instead of
// database-specific generated
package types
...
// Querier defines the complete set of database operations available across all supported databases.
// This interface uses database-agnostic types from the types package, allowing the same
// business logic to work with different database backends (SQLite, PostgreSQL, etc.).
//
// All methods in this interface mirror those generated by sqlc, but with types replaced:
//   - generated_sqlite.* types → *
//   - sql.NullInt64 → NullInt64
//   - interface{} (for nullable integers) → NullInt64
type Querier interface {
	...
	GetColumnNextID(ctx context.Context, id int64) (NullInt64, error)
