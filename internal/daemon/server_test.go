...
// TestReconnection_RestoresSubscription tests that when a client reconnects
// to the daemon, it properly restores its subscription to the previous project.
//
// Bug context: The fix includes subscription restoration in the reconnect() method.
//
// Note: This is a slower test (~4 seconds) due to reconnection delays.
func TestReconnection_RestoresSubscription(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping slow reconnection test in short mode")
	}
	...
	if err := client.Subscribe(5); err != nil {
		t.Fatalf("Failed to subscribe to project 5: %v", err)
	}
	time.Sleep(50 * time.Millisecond)
	...
	receivedEvent := waitForEvent(t, eventChan, 1*time.Second)
	if receivedEvent.ProjectID != 5 {
		t.Errorf("Expected event for project 5, got: %d", receivedEvent.ProjectID)
	}
	t.Logf("✓ Initial subscription to project 5 working")

	// Simulate disconnect by shutting down server
	t.Logf("Simulating disconnect...")
	if err := server.Shutdown(); err != nil {
	...
	time.Sleep(200 * time.Millisecond) // Extra time for listener to be ready

	// Client should automatically reconnect and restore subscription to project 5
	// Wait for reconnection (client has exponential backoff: 1s, 2s, 4s...)
	t.Logf("Waiting for client to reconnect and restore subscription...")
	time.Sleep(2 * time.Second)

	// Publish event for project 5 - should receive after reconnection
	event2 := events.Event{
		Type:      events.EventDatabaseChanged,
		ProjectID: 5,
		Timestamp: time.Now(),
	...
	receivedEvent = waitForEvent(t, eventChan, 2*time.Second)
	if receivedEvent.ProjectID != 5 {
		t.Errorf("Expected event for project 5 after reconnect, got: %d", receivedEvent.ProjectID)
	}
	t.Logf("✓ Subscription to project 5 restored after reconnection")

	// Publish event for different project - should NOT receive
	event3 := events.Event{
		Type:      events.EventDatabaseChanged,
		ProjectID: 3,
		Timestamp: time.Now(),
	...
	waitForNoEvent(t, eventChan, 300*time.Millisecond)
	t.Logf("✓ Correctly filtered out events for project 3 after reconnect")
}
