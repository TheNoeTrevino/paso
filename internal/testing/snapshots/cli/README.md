...
**51 golden files** in `testdata/help/`.

### Style Tests (`styles_test.go`)

Tests styled success messages across all 5 color schemes (default, monochrome, wave, dragon, lotus) to ensure consistent formatting.

Two test functions:

- `TestRenderSuccess_GoldenOutput` — 11 message variants x 5 schemes = 55 golden files
- `TestRenderSuccessWithDetails_GoldenOutput` — 5 message variants x 5 schemes = 25 golden files

**80 golden files** in `testdata/styles/`.
...
Covers 36 scenarios across all entity types:

- **Task** (18 tests): create, create with description, list, show, update, assign, estimate, comment, ready list, blocked list, in-progress list, move, in-progress move, to-ready, done, link, switch, delete
...
- `main_test.go` — `TestMain` entry point with goroutine leak detection via `goleak`
- `snapshots.NewHelper(t, dir)` — shared helper from `internal/testing/snapshots/snapshot.go` that handles golden file read/write and diff output
- `cli.SetupCLITest()` — creates an isolated in-memory SQLite database and returns a test `App` instance
- `cli.ExecuteCLICommandWithContext()` — runs a CLI command with a test `App` injected via context, capturing stdout/stderr
- `cli.CaptureOutputFunc()` — captures output from functions that write directly to `os.Stdout`/`os.Stderr` (uses a global mutex for safety)

## Commands Not Covered by E2E Tests
